class User < ActiveRecord::Base
  ...
  def video_summary
    ...
    weeks = 8
    list = self.videos.where("time_watched > :start_date", {start_date: weeks.week.ago.utc})

    list.each do |video|
      index = 0  
      while index < weeks do
        if video.time_watched > (index+1).week.ago.utc && video.time_watched < index.week.ago.utc 
          if output[:series][video.category]
            if output[:series][video.category][index]
              output[:series][video.category][index] += video.length / 60 
            else 
              output[:series][video.category][index] = video.length / 60
            end 
          else 
            output[:series][video.category] = Array.new(weeks, 0)
            output[:series][video.category][index] = video.length / 60
          end 
        end
        index += 1
